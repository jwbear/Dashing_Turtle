    ...
    def toggle_editable(self):
        if self.new_button.text() == "New":
            ...
            if not self.validate_inputs():
                ...
                return
            if self.secondary_input.text().strip() != "":
                if self.experiment_input.text().strip() == "":
                    QMessageBox.warning(self, "Validation Error", "If specifiying a control structure,"
                                                                  " the control experiment must be entered")
                    return
                if len(self.sequence_input.text().strip()) != len(self.secondary_input.text().strip()):
                    QMessageBox.warning(self, "Validation Error", "Sequence and secondary structure must be the same length.")
                    return
            self.add_seq()
            self.set_fields_enabled(False)
            self.id_input.setReadOnly(True)
...
class LoadBasecall(QFrame):
    ...
    def create_sample_graph(self, label, gpath=None):
        ...
        if isinstance(gpath, str) and os.path.exists(gpath):
        ...
        if isinstance(gpath, pd.DataFrame):
            if not gpath.empty:
                if 'position' in gpath.columns and 'mean_val' in gpath.columns:
                    ...
                    ax.set_xticks([])
                    ax.set_yticks([])
        elif isinstance(gpath, str) and os.path.exists(gpath):
            image = mpimg.imread(gpath)
            ax.imshow(image)
            ax.axis('off')
        else:
...
class PredictSection(QFrame):
    ...
    def create_sample_graph(self, image_path, label=None):
        # Load image first to get its aspect ratio
        ...
        fig = Figure(figsize=(6 * aspect_ratio, 6))  # Width scales with aspect
        canvas = FigureCanvas(fig)
        ...
        if image is not None:
            ax.imshow(image, aspect='auto')  # Let it scale naturally
        else:
    ...
    def run_prediction(self, label):
        ...
        #print('mod', str(seq_mod))
        #print('unmod', str(seq_unmod))
        if label == "Predict Modifications":
            if str(seq_mod)==str(seq_unmod):
                image1, image2 = predict.run_predict(unmod_lids=self.lid_unmod,lids=self.lid_mod, continue_reads=False,
...
class CreateLandscapeSection(QFrame):
    ...
    def create_landscape(self):
        ...
        # print('unmod', str(seq_unmod))
        if str(seq_mod) == str(seq_unmod):
            images = landscape.run_landscape(unmod_lid=self.lid_unmod, lid=self.lid_mod,
    ...
    def create_sample_graph(self, image_path, label=None):
        # Load image first to check if it exists (no need to get aspect ratio here)
        if os.path.exists(image_path):
            image = mpimg.imread(image_path)
        else:
            image = None
...
class MainApp(QWidget):
    ...
    def init_ui(self):
        ...
        layout.addWidget(PredictSection())
        layout.addWidget(CreateLandscapeSection())
