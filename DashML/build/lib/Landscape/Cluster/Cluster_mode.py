...
def get_clusters(X, seq, read_list, tit, clust_num, plot=False):
...
def clustermap(X, seq='HCV', tit='Hamming'):
    # cannot contain nan
    X = np.nan_to_num(X)
...
def corrmatrix(seq='HCV', data=None, tit="Hamming"):
    g = sns.heatmap(np.corrcoef(data))
    g.set_xlabel("Reads")
    g.set_ylabel("Reads")
    g.set_title( "Hamming Read Correlations")
...
#### kmode to get centroids ####
def get_centroids(data, clusters, seq):
    # set cluster labels to reads
    df = data.merge(clusters, on=['read_id'], how='left')
    df['method'] = 'hamming'
    ...
    for i in clusters['cluster'].unique():
        ...
        lids = dt['LID'].unique()
        dt = dt[['read_id', 'position', 'Predict']]
        dt = dt.sort_values(by=['read_id', 'position'])
        dt = (dt.pivot(index=["read_id"], columns="position", values="Predict")
               .rename_axis(columns=None)
               .reset_index())
        ...
        #unique, counts = np.unique(km.cluster_centroids_, return_counts=True)
        #print(unique, counts)
        for lid in lids:
            df_centroid = pd.concat([df_centroid, pd.DataFrame({'LID': lid, 'cluster': i, 'position': np.arange(0, seqlen+1),
                                                            'centroid': np.array(km.cluster_centroids_).flatten()})])
    df_centroid['method'] = 'hamming'
...
def den_array(df, clust_num, plot=True):
    ...
    clusters, img_path_dend = get_clusters(X=distk, read_list=read_list, seq=seq, tit=tit, plot=plot, clust_num=clust_num)
    #centroids
    get_centroids(data=df, clusters=clusters, seq=seq)
    if plot:
        ...
        img_corr = corrmatrix(seq=seq, data=reads, tit=tit)
        return img_path_dend, img_path_heatmap, img_corr
    del df
    return None
